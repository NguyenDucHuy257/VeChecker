...
class TelegramController:
    ...
    def handle_update(self, update: dict[str, Any]) -> None:
        ...
        if command == "/logout":
            ...
            logged_out = self.source_pool.logout()
        ...
        if command == "/login":
            ...
            self.bot.send_message(
                chat_id,
                "Gửi username tài khoản nguồn dùng chung. Bot sẽ xóa tin nhắn "
                "ngay sau khi nhận. Dùng /cancel để hủy.",
            )
            return
        if (
            user.role is UserRole.ADMIN
            and self._get_login_draft(telegram_user_id) is not None
        ):
            ...
            self.bot.send_message(
                ...
                " | ".join(
                    (
                        ...
                        f"shared_sessions={self.source_pool.authenticated_users}",
        ...
        if command in {"/captcha", "/refresh_captcha"}:
            ...
            try:
                response = (
                    self.source_pool.submit_captcha(argument)
                    if command == "/captcha"
                    else self.source_pool.refresh_captcha()
                )
    ...
    def _send_login_response(self, chat_id: int, response: SourceLoginResponse) -> None:
        ...
        if response.prompt is not None:
            ...
            self.bot.send_photo(
                ...
                prompt.image,
                caption=(
                    f"CAPTCHA session nguồn chung {prompt.attempt}/{prompt.maximum}. "
    ...
    def _handle_login_secret(
        ...
        self._clear_login_draft(telegram_user_id)
        try:
            response = self.source_pool.login(username, value)
        except VrServiceError as exc:
            self.bot.send_message(
                chat_id,
